...
describe('Talent Damage Bonuses', () => {
  ...
  describe('CombatHelper.getWeaponDamageBonus', () => {
    it('returns 2 for melee when actor has Crushing Blow', () => {
      const actor = {
        system: { modifiers: [] },
        effects: [],
        items: [{
      ...
      expect(CombatHelper.getWeaponDamageBonus(actor, true)).toBe(2);
    });

    it('returns 2 for ranged when actor has Mighty Shot', () => {
      const actor = {
        system: { modifiers: [] },
        effects: [],
        items: [{
      ...
      expect(CombatHelper.getWeaponDamageBonus(actor, false)).toBe(2);
    });

    it('returns 0 when actor has no weapon-damage modifiers', () => {
      const actor = {
        system: { modifiers: [] },
        effects: [],
        items: []
      };
      expect(CombatHelper.getWeaponDamageBonus(actor, true)).toBe(0);
    });

    it('sums multiple weapon-damage modifiers', () => {
      const actor = {
        system: { modifiers: [] },
        effects: [],
        items: [{
          ...
          type: 'talent',
          name: 'Hunter of Aliens',
          system: {
            modifiers: [{
              name: 'Hunter of Aliens - Damage',
              modifier: 2,
              effectType: 'weapon-damage',
              enabled: true
            }]
      ...
      expect(CombatHelper.getWeaponDamageBonus(actor, true)).toBe(4);
    });
  });

  describe('buildDamageFormula - weaponDamageBonus', () => {
    it('adds weapon damage bonus to melee formula', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        baseDmg: '1d10+5', degreesOfSuccess: 0, isMelee: true, strBonus: 4, hitIndex: 0, weaponDamageBonus: 2
      });
      expect(formula).toBe('1d10+5 + 4 + 2');
    });

    it('adds weapon damage bonus to ranged formula', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        baseDmg: '2d10+5', degreesOfSuccess: 0, isMelee: false, strBonus: 0, hitIndex: 0, weaponDamageBonus: 2
      });
    ...
    it('does not add when bonus is 0', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        baseDmg: '1d10+5', degreesOfSuccess: 0, isMelee: true, strBonus: 4, hitIndex: 0, weaponDamageBonus: 0
      });
    ...
    it('stacks with Power Fist STR doubling', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        baseDmg: '2d10', degreesOfSuccess: 0, isMelee: true, strBonus: 10, hitIndex: 0, isPowerFist: true, weaponDamageBonus: 2
      });
    ...
    it('stacks with Lightning Claw bonus', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        ...
        isLightningClaw: true, hasLightningClawPair: false, weaponDamageBonus: 2
      });
    ...
    it('stacks with Accurate bonus damage', () => {
      const formula = CombatDialogHelper.buildDamageFormula({
        ...
        isAccurate: true, isAiming: true, isSingleShot: true, weaponDamageBonus: 2
      });
